/*! \file
    \brief Прокси DLL для подмены оригинальной DLL, подменяет весь API sqlite3, логгирует вызов и отправляет в оригинальную, которая должна лежать рядом с именем e_sqlite3_org.dll
